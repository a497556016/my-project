    ...
    <div class="create-form">
        ...
        <div class="right-content" :style="{height: contentHeight}">
            <div class="input-item title">
                <input :style="{color: formData.meta.titleColor}" type="text" placeholder="请输入标题" v-model="formData.title" />
                <a-divider/>
                <div class="item-footer">
                    <div class="action" @click="configItem(-1)">
            ...
            <div v-if="formData.formItems.length == 0" class="drag-content" @drop="dropHandler" @dragover="allowDrop">
                从左边控件选择控件拖动到该区域
            </div>
            <div v-else :class="['input-item', (settingItem&&settingItem.index==index)?'item-bordered':'']" v-for="(item, index) in formData.formItems" :key="index" :data-index="index" @drop="dropHandler" @dragover="allowDrop" @dragleave="dragLeaveHandler">
                <question :seq="index+1" :placeholder="'['+item.name+'] 请在此输入你的问题'" v-model="item.question"></question>
                <component :is="item.component" v-model="item.meta"></component>
                <a-divider/>
                <item-footer :item="item" :index="index" @copy="formData.formItems.splice(index+1, 0, item)" @remove="onRemove(index)" @move="moveItem(index)" @setting="configItem(index)" @hide="onHide(item)"></item-footer>
...
<script>
    ...
    import MFormSetting from '@/components/formEditItems/setting/Form';
    ...
    export default {
        ...
        data(){
            return {
                ...
                formItems: [
                    {
                        name: '单选题', component: 'm-radio', question: '', meta: {
                            options: [
                    ...
                    {
                        name: '下拉选择题', component: 'm-select', question: '', meta: {
                            options: [
                                {label: '选项', value: 0},
                                {label: '其他', value: 1}
                            ]
        ...
        methods: {
            ...
            dropHandler(e){
                ...
                let index = 0;
                if(this.formData.formItems.length == 0) {
                    this.formData.formItems.push(this.dragItem);
                }else{
                    ...
                    console.log(index);
                    this.formData.formItems.splice(index, 0, this.dragItem);
                }
            ...
            moveItem(index){
                ...
                const item = this.formData.formItems.splice(index, 1)[0];
                ...
                // console.log(index);
                this.formData.formItems.splice(targetIndex, 0, item);
            },
            allowDrop(e){
                e.preventDefault();
                console.log(e)
            ...
            configItem(index) {
                if(index == -1){
                    //表单属性设置
                    this.settingItem = {index: index, component: 'm-form-setting', config: this.formData.meta}
                }else {
                    const item = this.formData.formItems[index];
            ...
            onRemove(index) {
                this.formData.formItems.splice(index, 1);
                if(this.settingItem.index == index) {
                    this.settingItem = undefined;
                }
            },
            save(){
                console.log(this.formData.formItems);
                ...
                const noQesItem = this.formData.formItems.find(item => !item.disabled&&!item.question);
                if(noQesItem){
                    this.$error({
                        content: '必须输入问题描述！'
                    })
